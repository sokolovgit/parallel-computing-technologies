...
- `lab6/project` is MPI (Open MPI) coursework: C under `src/` (`task1_p2p.c`, `task2_mm_blocking.c`, `task3_mm_nonblocking.c`, …), output in `bin/`; `justfile` has `task1` (`mpirun -np 2`), `task2` / `task3` (blocking vs non-blocking matrix multiply, `mpirun -np "$MP_PROCS"` default 4, optional matrix args); report draft in `lab6/docs/report/lab6.md`.
