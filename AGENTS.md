...
- `lab6/project` is MPI (Open MPI) coursework: C under `src/` (`task1_p2p.c`, `task2_mm_blocking.c`, …), output in `bin/`; `justfile` has `task1` (`mpirun -np 2`) and `task2` (builds `task2_mm_blocking`, runs `mpirun -np "$MP_PROCS"` default 4, optional matrix args); report draft in `lab6/docs/report/lab6.md`.
